...
class TestTheOrderIsExplicit:
    def test_production_runs_in_this_order_and_no_other(self):
        ...
        of it with a commodity, `epidemic` caps whatever came out, and the
        robber takes the lot. Any other sequence pays a different number.
        """
        assert [
            modifier.rule_id
            for modifier in modifiers_module.registered(modifiers_module.PRODUCTION)
        ] == ['city_production', 'commodities', 'epidemic', 'robber']
